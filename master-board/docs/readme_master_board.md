...
1. Flashare via USB lo sketch di avvio/recovery (§7.1) su una scheda ESP32‐C3 con display OLED da 0,42 pollici WiFi Bluetooth.
2. Verificare che l'AP WiFi del master parta correttamente, che tra le reti WiFi esiste quella con SSID = **88888888**, e che l'endpoint `/update` sia raggiungibile (§6).
3. Da menù attivare aggiornamento OTA 
4.Aggiornare il firmware caricando il file 4MASTER32_displ_pir_wdog_100.bin (N.B. 100 è la versione, potrebbe essere rilasciata un upgrade con altro numero) .
5. Da quel momento in poi, tutti gli aggiornamenti successivi possono avvenire via OTA (§6), senza bisogno di ricollegare la scheda via USB.
